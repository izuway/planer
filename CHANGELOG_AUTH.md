...
**Обязательная верификация email** - пользователи не могут получить доступ к приложению без подтверждения email адреса.
...
- `src/context/AuthContext.tsx` - Auth context с функциями:
  - `signup(email, password, displayName)` - автоматически отправляет verification email
  - `login(email, password)`
  - `logout()`
  - `getIdToken()` - получение актуального JWT
  - `sendVerificationEmail()` - повторная отправка письма верификации
  - `reloadUser()` - обновление user state для проверки emailVerified
...
**Email Verification:**
- `src/components/EmailVerification.tsx` - страница верификации email
  - Показывается пользователям с неподтвержденным email
  - Кнопка повторной отправки письма (cooldown 60 секунд)
  - Кнопка проверки статуса верификации
  - Информация о том куда отправлено письмо
  - Возможность выйти и зарегистрироваться заново
...
- `src/components/ProtectedRoute.tsx` - HOC для защиты routes
  ...
  - **Показывает EmailVerification если email не подтвержден**
  - Доступ к приложению только для verified users
...
1. **Email Verification (NEW!)**
   ...
   - Автоматическая отправка verification email при регистрации
   - Проверка `user.emailVerified` статуса
   - Блокировка доступа для unverified users
...
3. **Route Protection**
   ...
   - **Двойная проверка: authentication + email verification**
...
- ✅ **После регистрации/логина с неподтвержденным email показывается EmailVerification**
- ✅ **Письмо с подтверждением отправляется автоматически**
- ✅ **После подтверждения email открывается основное приложение**
...
- [x] **Email verification** ✅ РЕАЛИЗОВАНО!
